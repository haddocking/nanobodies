...
\begin{figure*}
    ...
    \caption{\textcolor{Red}{\textbf{Graphical representation of the nanobody-antigen complex prediction pipeline.} First, nanobody conformations are generated using AlphaFold (AF2), AlphaFold2-Multimer (AF2-multi) and ImmuneBuilder (IB). Those are then clustered based on the similarity of the CDR3 loop and passed to HADDOCK for ensemble-based information-driven docking using CDR loop residues and some knowledge of the epitope to define ambiguous interaction restraints to drive the modelling process.}}
    \label{fig:workflow}
\end{figure*}
